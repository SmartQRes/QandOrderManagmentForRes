...
//            var resid:NSNumber = item.uss_res_id
            prefs.setObject(MyVariables.username, forKey: "USERNAME")
            prefs.setInteger(1, forKey: "ISLOGGEDIN")
            prefs.setObject(MyVariables.branchAdminUserType, forKey: "USERTYPE")
            prefs.setObject(MyVariables.defaultBranch.bra_id, forKey: "BRANCHID")
            prefs.setObject(MyVariables.defaultBranch.bra_res_id, forKey: "RESID")
...
//        }
    ...
    func getAuthenStaff(username: String,password: String, uiView : LogInViewController) -> Void{
       ...
       // self.instance.pullItems()
        ...
        print("Authen Method")
        if(username == MyVariables.username){
            if(password == MyVariables.password){
